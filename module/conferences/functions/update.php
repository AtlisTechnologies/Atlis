...
$stmt = $pdo->prepare('UPDATE module_conferences SET user_updated=?, title=?, type=?, topic=?, mode=?, venue=?, schedule=?, description=?, organizers=?, sponsors=?, tags=?, ticket_options=?, custom_fields=?, privacy=? WHERE id=?');
$stmt->execute([$this_user_id,$title,$type,$topic,$mode,$venue,$schedule,$description,$organizers,$sponsors,$tags,$ticket_options,$custom_fields,$privacy,$id]);
...
if (!empty($_FILES['images']['tmp_name'][0])) {
  ...
  if ($paths) {
    ...
    $imgs = $existing ? json_decode($existing, true) : [];
    $imgs = array_merge($imgs, $paths);
    $pdo->prepare('UPDATE module_conferences SET images=? WHERE id=?')->execute([json_encode($imgs),$id]);
  }
}
