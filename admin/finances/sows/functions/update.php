...
$stmt = $pdo->prepare('UPDATE module_sows SET user_updated=:uid,title=:title,summary=:summary,organization_id=:org,agency_id=:agency,division_id=:division,project_id=:project,status_id=:status,signatures=:sigs WHERE id=:id');
$stmt->execute([
  ':uid'=>$this_user_id,
  ':title'=>$title,
...
admin_audit_log($pdo,$this_user_id,'module_sows',$id,'UPDATE',null,json_encode(['title'=>$title]));
header('Location: ../edit.php?id='.$id);
exit;
